WITH CTE_VOCAB_MAP AS (
       ...
       FROM mimic_iv_ed.CONCEPT C
             JOIN mimic_iv_ed.CONCEPT_RELATIONSHIP CR
                        ON C.CONCEPT_ID = CR.CONCEPT_ID_1
                        AND CR.invalid_reason IS NULL
                        AND lower(cr.relationship_id) = 'maps to'
              JOIN mimic_iv_ed.CONCEPT C1
       ...
       FROM mimic_iv_ed.source_to_concept_map stcm
              LEFT OUTER JOIN mimic_iv_ed.CONCEPT c1
                     ON c1.concept_id = stcm.source_concept_id
              LEFT OUTER JOIN mimic_iv_ed.CONCEPT c2
...
FROM CTE_VOCAB_MAP
/*EXAMPLE FILTERS*/
WHERE TARGET_VOCABULARY_ID IN ('RxNorm','RxNorm Extension') 