initFramework <- function() {
  ...
  defaults$subject_id <- ''
  defaults$gender <- 'F'
  defaults$anchor_age <- '0'
  defaults$anchor_year <- '2153'
  defaults$anchor_year_group <- '2008 - 2010'
  defaults$dod <- ''
  assign('patients', defaults, envir = frameworkContext$defaultValues)

  defaults <- list()
  defaults$subject_id <- '10714009'
  defaults$hadm_id <- ''
  defaults$admittime <- ''
  defaults$dischtime <- ''
  defaults$deathtime <- ''
  defaults$admission_type <- 'EW EMER.'
  defaults$admission_location <- 'EMERGENCY ROOM'
  defaults$discharge_location <- 'HOME'
  defaults$insurance <- 'Other'
  defaults$language <- 'ENGLISH'
  defaults$marital_status <- 'MARRIED'
  defaults$ethnicity <- 'WHITE'
  defaults$edregtime <- ''
  defaults$edouttime <- ''
  defaults$hospital_expire_flag <- '0'
  assign('admissions', defaults, envir = frameworkContext$defaultValues)

  defaults <- list()
  defaults$subject_id <- '18358138'
  defaults$hadm_id <- '27373827'
  defaults$stay_id <- ''
  defaults$first_careunit <- 'Medical Intensive Care Unit (MICU)'
  defaults$last_careunit <- 'Medical Intensive Care Unit (MICU)'
  defaults$intime <- ''
  defaults$outtime <- ''
  defaults$los <- '1.0220717592592592'
  assign('icustays', defaults, envir = frameworkContext$defaultValues)

  defaults <- list()
  defaults$subject_id <- '12985376'
  defaults$hadm_id <- '26693268'
  defaults$seq_num <- '1'
  defaults$icd_code <- '4019'
  defaults$icd_version <- '9'
  assign('diagnoses_icd', defaults, envir = frameworkContext$defaultValues)
...
set_defaults_patients <- function(subject_id, gender, anchor_age, anchor_year, anchor_year_group, dod) {
  ...
  if (!missing(anchor_age)) {
    defaults$anchor_age <- anchor_age
  }
  if (!missing(anchor_year)) {
    defaults$anchor_year <- anchor_year
  }
  if (!missing(anchor_year_group)) {
    defaults$anchor_year_group <- anchor_year_group
  }
  if (!missing(dod)) {
    defaults$dod <- dod
  }
...
set_defaults_admissions <- function(subject_id, hadm_id, admittime, dischtime, deathtime, admission_type, admission_location, discharge_location, insurance, language, marital_status, ethnicity, edregtime, edouttime, hospital_expire_flag) {
  defaults <- get('admissions', envir = frameworkContext$defaultValues)
  if (!missing(subject_id)) {
    defaults$subject_id <- subject_id
  }
  if (!missing(hadm_id)) {
    defaults$hadm_id <- hadm_id
  }
  if (!missing(admittime)) {
    defaults$admittime <- admittime
  }
  if (!missing(dischtime)) {
    defaults$dischtime <- dischtime
  }
  if (!missing(deathtime)) {
    defaults$deathtime <- deathtime
  }
  if (!missing(admission_type)) {
    defaults$admission_type <- admission_type
  }
  if (!missing(admission_location)) {
    defaults$admission_location <- admission_location
  }
  if (!missing(discharge_location)) {
    defaults$discharge_location <- discharge_location
  }
  if (!missing(insurance)) {
    defaults$insurance <- insurance
  }
  if (!missing(language)) {
  ...
  if (!missing(edregtime)) {
    defaults$edregtime <- edregtime
  }
  if (!missing(edouttime)) {
    defaults$edouttime <- edouttime
  }
  if (!missing(hospital_expire_flag)) {
    defaults$hospital_expire_flag <- hospital_expire_flag
  }
  assign('admissions', defaults, envir = frameworkContext$defaultValues)
  invisible(defaults)
}

set_defaults_icustays <- function(subject_id, hadm_id, stay_id, first_careunit, last_careunit, intime, outtime, los) {
  defaults <- get('icustays', envir = frameworkContext$defaultValues)
  if (!missing(subject_id)) {
    defaults$subject_id <- subject_id
  }
  if (!missing(hadm_id)) {
  ...
  if (!missing(stay_id)) {
    defaults$stay_id <- stay_id
  }
  if (!missing(first_careunit)) {
    defaults$first_careunit <- first_careunit
  }
  if (!missing(last_careunit)) {
    defaults$last_careunit <- last_careunit
  }
  if (!missing(intime)) {
    defaults$intime <- intime
  }
  if (!missing(outtime)) {
    defaults$outtime <- outtime
  }
  if (!missing(los)) {
    defaults$los <- los
  }
...
set_defaults_diagnoses_icd <- function(subject_id, hadm_id, seq_num, icd_code, icd_version) {
  defaults <- get('diagnoses_icd', envir = frameworkContext$defaultValues)
  if (!missing(subject_id)) {
    defaults$subject_id <- subject_id
  }
  if (!missing(hadm_id)) {
...
get_defaults_icustays <- function() {
  defaults <- get('icustays', envir = frameworkContext$defaultValues)
  return(defaults)
}

get_defaults_diagnoses_icd <- function() {
  defaults <- get('diagnoses_icd', envir = frameworkContext$defaultValues)
  return(defaults)
}
...
add_patients <- function(subject_id, gender, anchor_age, anchor_year, anchor_year_group, dod) {
  defaults <- get('patients', envir = frameworkContext$defaultValues)
  fields <- c()
  values <- c()
  if (missing(subject_id)) {
    ...
    frameworkContext$sourceFieldsTested <- c(frameworkContext$sourceFieldsTested, 'patients.subject_id')
  }
  ...
  if (missing(anchor_age)) {
    anchor_age <- defaults$anchor_age
  } else {
    frameworkContext$sourceFieldsTested <- c(frameworkContext$sourceFieldsTested, 'patients.anchor_age')
  }
  fields <- c(fields, "anchor_age")
  values <- c(values, if (is.null(anchor_age)) "NULL" else if (is(anchor_age, "subQuery")) paste0("(", as.character(anchor_age), ")") else paste0("'", as.character(anchor_age), "'"))

  if (missing(anchor_year)) {
    anchor_year <- defaults$anchor_year
  } else {
    frameworkContext$sourceFieldsTested <- c(frameworkContext$sourceFieldsTested, 'patients.anchor_year')
  }
  fields <- c(fields, "anchor_year")
  values <- c(values, if (is.null(anchor_year)) "NULL" else if (is(anchor_year, "subQuery")) paste0("(", as.character(anchor_year), ")") else paste0("'", as.character(anchor_year), "'"))

  if (missing(anchor_year_group)) {
    anchor_year_group <- defaults$anchor_year_group
  } else {
    frameworkContext$sourceFieldsTested <- c(frameworkContext$sourceFieldsTested, 'patients.anchor_year_group')
  }
  fields <- c(fields, "anchor_year_group")
  values <- c(values, if (is.null(anchor_year_group)) "NULL" else if (is(anchor_year_group, "subQuery")) paste0("(", as.character(anchor_year_group), ")") else paste0("'", as.character(anchor_year_group), "'"))

  if (missing(dod)) {
    dod <- defaults$dod
  } else {
    frameworkContext$sourceFieldsTested <- c(frameworkContext$sourceFieldsTested, 'patients.dod')
  }
  fields <- c(fields, "dod")
  values <- c(values, if (is.null(dod)) "NULL" else if (is(dod, "subQuery")) paste0("(", as.character(dod), ")") else paste0("'", as.character(dod), "'"))
...
add_admissions <- function(subject_id, hadm_id, admittime, dischtime, deathtime, admission_type, admission_location, discharge_location, insurance, language, marital_status, ethnicity, edregtime, edouttime, hospital_expire_flag) {
  defaults <- get('admissions', envir = frameworkContext$defaultValues)
  fields <- c()
  values <- c()
  if (missing(subject_id)) {
    ...
    frameworkContext$sourceFieldsTested <- c(frameworkContext$sourceFieldsTested, 'admissions.subject_id')
  }
  fields <- c(fields, "subject_id")
  values <- c(values, if (is.null(subject_id)) "NULL" else if (is(subject_id, "subQuery")) paste0("(", as.character(subject_id), ")") else paste0("'", as.character(subject_id), "'"))

  if (missing(hadm_id)) {
    hadm_id <- defaults$hadm_id
  } else {
    frameworkContext$sourceFieldsTested <- c(frameworkContext$sourceFieldsTested, 'admissions.hadm_id')
  }
  fields <- c(fields, "hadm_id")
  values <- c(values, if (is.null(hadm_id)) "NULL" else if (is(hadm_id, "subQuery")) paste0("(", as.character(hadm_id), ")") else paste0("'", as.character(hadm_id), "'"))

  if (missing(admittime)) {
    admittime <- defaults$admittime
  } else {
    frameworkContext$sourceFieldsTested <- c(frameworkContext$sourceFieldsTested, 'admissions.admittime')
  }
  fields <- c(fields, "admittime")
  values <- c(values, if (is.null(admittime)) "NULL" else if (is(admittime, "subQuery")) paste0("(", as.character(admittime), ")") else paste0("'", as.character(admittime), "'"))

  if (missing(dischtime)) {
    dischtime <- defaults$dischtime
  } else {
    frameworkContext$sourceFieldsTested <- c(frameworkContext$sourceFieldsTested, 'admissions.dischtime')
  }
  fields <- c(fields, "dischtime")
  values <- c(values, if (is.null(dischtime)) "NULL" else if (is(dischtime, "subQuery")) paste0("(", as.character(dischtime), ")") else paste0("'", as.character(dischtime), "'"))

  if (missing(deathtime)) {
    deathtime <- defaults$deathtime
  } else {
    frameworkContext$sourceFieldsTested <- c(frameworkContext$sourceFieldsTested, 'admissions.deathtime')
  }
  fields <- c(fields, "deathtime")
  values <- c(values, if (is.null(deathtime)) "NULL" else if (is(deathtime, "subQuery")) paste0("(", as.character(deathtime), ")") else paste0("'", as.character(deathtime), "'"))

  if (missing(admission_type)) {
    admission_type <- defaults$admission_type
  } else {
    frameworkContext$sourceFieldsTested <- c(frameworkContext$sourceFieldsTested, 'admissions.admission_type')
  }
  fields <- c(fields, "admission_type")
  values <- c(values, if (is.null(admission_type)) "NULL" else if (is(admission_type, "subQuery")) paste0("(", as.character(admission_type), ")") else paste0("'", as.character(admission_type), "'"))

  if (missing(admission_location)) {
    admission_location <- defaults$admission_location
  } else {
    frameworkContext$sourceFieldsTested <- c(frameworkContext$sourceFieldsTested, 'admissions.admission_location')
  }
  fields <- c(fields, "admission_location")
  values <- c(values, if (is.null(admission_location)) "NULL" else if (is(admission_location, "subQuery")) paste0("(", as.character(admission_location), ")") else paste0("'", as.character(admission_location), "'"))

  if (missing(discharge_location)) {
    discharge_location <- defaults$discharge_location
  } else {
    frameworkContext$sourceFieldsTested <- c(frameworkContext$sourceFieldsTested, 'admissions.discharge_location')
  }
  fields <- c(fields, "discharge_location")
  values <- c(values, if (is.null(discharge_location)) "NULL" else if (is(discharge_location, "subQuery")) paste0("(", as.character(discharge_location), ")") else paste0("'", as.character(discharge_location), "'"))

  if (missing(insurance)) {
    insurance <- defaults$insurance
  } else {
    frameworkContext$sourceFieldsTested <- c(frameworkContext$sourceFieldsTested, 'admissions.insurance')
  }
  fields <- c(fields, "insurance")
  values <- c(values, if (is.null(insurance)) "NULL" else if (is(insurance, "subQuery")) paste0("(", as.character(insurance), ")") else paste0("'", as.character(insurance), "'"))

  if (missing(language)) {
    language <- defaults$language
  } else {
    frameworkContext$sourceFieldsTested <- c(frameworkContext$sourceFieldsTested, 'admissions.language')
  }
  fields <- c(fields, "language")
  values <- c(values, if (is.null(language)) "NULL" else if (is(language, "subQuery")) paste0("(", as.character(language), ")") else paste0("'", as.character(language), "'"))

  if (missing(marital_status)) {
    marital_status <- defaults$marital_status
  } else {
    frameworkContext$sourceFieldsTested <- c(frameworkContext$sourceFieldsTested, 'admissions.marital_status')
  }
  fields <- c(fields, "marital_status")
  values <- c(values, if (is.null(marital_status)) "NULL" else if (is(marital_status, "subQuery")) paste0("(", as.character(marital_status), ")") else paste0("'", as.character(marital_status), "'"))

  if (missing(ethnicity)) {
    ethnicity <- defaults$ethnicity
  } else {
    frameworkContext$sourceFieldsTested <- c(frameworkContext$sourceFieldsTested, 'admissions.ethnicity')
  }
  ...
  if (missing(edregtime)) {
    edregtime <- defaults$edregtime
  } else {
    frameworkContext$sourceFieldsTested <- c(frameworkContext$sourceFieldsTested, 'admissions.edregtime')
  }
  fields <- c(fields, "edregtime")
  values <- c(values, if (is.null(edregtime)) "NULL" else if (is(edregtime, "subQuery")) paste0("(", as.character(edregtime), ")") else paste0("'", as.character(edregtime), "'"))

  if (missing(edouttime)) {
    edouttime <- defaults$edouttime
  } else {
    frameworkContext$sourceFieldsTested <- c(frameworkContext$sourceFieldsTested, 'admissions.edouttime')
  }
  fields <- c(fields, "edouttime")
  values <- c(values, if (is.null(edouttime)) "NULL" else if (is(edouttime, "subQuery")) paste0("(", as.character(edouttime), ")") else paste0("'", as.character(edouttime), "'"))

  if (missing(hospital_expire_flag)) {
    hospital_expire_flag <- defaults$hospital_expire_flag
  } else {
    frameworkContext$sourceFieldsTested <- c(frameworkContext$sourceFieldsTested, 'admissions.hospital_expire_flag')
  }
  fields <- c(fields, "hospital_expire_flag")
  values <- c(values, if (is.null(hospital_expire_flag)) "NULL" else if (is(hospital_expire_flag, "subQuery")) paste0("(", as.character(hospital_expire_flag), ")") else paste0("'", as.character(hospital_expire_flag), "'"))
...
add_icustays <- function(subject_id, hadm_id, stay_id, first_careunit, last_careunit, intime, outtime, los) {
  defaults <- get('icustays', envir = frameworkContext$defaultValues)
  fields <- c()
  values <- c()
  if (missing(subject_id)) {
    ...
    frameworkContext$sourceFieldsTested <- c(frameworkContext$sourceFieldsTested, 'icustays.subject_id')
  }
  fields <- c(fields, "subject_id")
  values <- c(values, if (is.null(subject_id)) "NULL" else if (is(subject_id, "subQuery")) paste0("(", as.character(subject_id), ")") else paste0("'", as.character(subject_id), "'"))
  ...
  if (missing(hadm_id)) {
    ...
    frameworkContext$sourceFieldsTested <- c(frameworkContext$sourceFieldsTested, 'icustays.hadm_id')
  }
  fields <- c(fields, "hadm_id")
  values <- c(values, if (is.null(hadm_id)) "NULL" else if (is(hadm_id, "subQuery")) paste0("(", as.character(hadm_id), ")") else paste0("'", as.character(hadm_id), "'"))

  if (missing(stay_id)) {
    stay_id <- defaults$stay_id
  } else {
    frameworkContext$sourceFieldsTested <- c(frameworkContext$sourceFieldsTested, 'icustays.stay_id')
  }
  fields <- c(fields, "stay_id")
  values <- c(values, if (is.null(stay_id)) "NULL" else if (is(stay_id, "subQuery")) paste0("(", as.character(stay_id), ")") else paste0("'", as.character(stay_id), "'"))

  if (missing(first_careunit)) {
    first_careunit <- defaults$first_careunit
  } else {
    frameworkContext$sourceFieldsTested <- c(frameworkContext$sourceFieldsTested, 'icustays.first_careunit')
  }
  fields <- c(fields, "first_careunit")
  values <- c(values, if (is.null(first_careunit)) "NULL" else if (is(first_careunit, "subQuery")) paste0("(", as.character(first_careunit), ")") else paste0("'", as.character(first_careunit), "'"))

  if (missing(last_careunit)) {
    last_careunit <- defaults$last_careunit
  } else {
    frameworkContext$sourceFieldsTested <- c(frameworkContext$sourceFieldsTested, 'icustays.last_careunit')
  }
  fields <- c(fields, "last_careunit")
  values <- c(values, if (is.null(last_careunit)) "NULL" else if (is(last_careunit, "subQuery")) paste0("(", as.character(last_careunit), ")") else paste0("'", as.character(last_careunit), "'"))

  if (missing(intime)) {
    intime <- defaults$intime
  } else {
    frameworkContext$sourceFieldsTested <- c(frameworkContext$sourceFieldsTested, 'icustays.intime')
  }
  fields <- c(fields, "intime")
  values <- c(values, if (is.null(intime)) "NULL" else if (is(intime, "subQuery")) paste0("(", as.character(intime), ")") else paste0("'", as.character(intime), "'"))

  if (missing(outtime)) {
    outtime <- defaults$outtime
  } else {
    frameworkContext$sourceFieldsTested <- c(frameworkContext$sourceFieldsTested, 'icustays.outtime')
  }
  fields <- c(fields, "outtime")
  values <- c(values, if (is.null(outtime)) "NULL" else if (is(outtime, "subQuery")) paste0("(", as.character(outtime), ")") else paste0("'", as.character(outtime), "'"))

  if (missing(los)) {
    los <- defaults$los
  } else {
    frameworkContext$sourceFieldsTested <- c(frameworkContext$sourceFieldsTested, 'icustays.los')
  }
  fields <- c(fields, "los")
  values <- c(values, if (is.null(los)) "NULL" else if (is(los, "subQuery")) paste0("(", as.character(los), ")") else paste0("'", as.character(los), "'"))

  inserts <- list(testId = frameworkContext$testId, testDescription = frameworkContext$testDescription, table = "icustays", fields = fields, values = values)
  frameworkContext$inserts[[length(frameworkContext$inserts) + 1]] <- inserts
  invisible(NULL)
}

add_diagnoses_icd <- function(subject_id, hadm_id, seq_num, icd_code, icd_version) {
  defaults <- get('diagnoses_icd', envir = frameworkContext$defaultValues)
  fields <- c()
  values <- c()
  if (missing(subject_id)) {
    ...
    frameworkContext$sourceFieldsTested <- c(frameworkContext$sourceFieldsTested, 'diagnoses_icd.subject_id')
  }
  fields <- c(fields, "subject_id")
  values <- c(values, if (is.null(subject_id)) "NULL" else if (is(subject_id, "subQuery")) paste0("(", as.character(subject_id), ")") else paste0("'", as.character(subject_id), "'"))
  ...
  if (missing(hadm_id)) {
    ...
    frameworkContext$sourceFieldsTested <- c(frameworkContext$sourceFieldsTested, 'diagnoses_icd.hadm_id')
  }
  fields <- c(fields, "hadm_id")
  values <- c(values, if (is.null(hadm_id)) "NULL" else if (is(hadm_id, "subQuery")) paste0("(", as.character(hadm_id), ")") else paste0("'", as.character(hadm_id), "'"))

  if (missing(seq_num)) {
    seq_num <- defaults$seq_num
  } else {
    frameworkContext$sourceFieldsTested <- c(frameworkContext$sourceFieldsTested, 'diagnoses_icd.seq_num')
  }
  fields <- c(fields, "seq_num")
  values <- c(values, if (is.null(seq_num)) "NULL" else if (is(seq_num, "subQuery")) paste0("(", as.character(seq_num), ")") else paste0("'", as.character(seq_num), "'"))

  if (missing(icd_code)) {
    icd_code <- defaults$icd_code
  } else {
    frameworkContext$sourceFieldsTested <- c(frameworkContext$sourceFieldsTested, 'diagnoses_icd.icd_code')
  }
  ...
  if (missing(icd_version)) {
    ...
    frameworkContext$sourceFieldsTested <- c(frameworkContext$sourceFieldsTested, 'diagnoses_icd.icd_version')
  }
  ...
  inserts <- list(testId = frameworkContext$testId, testDescription = frameworkContext$testDescription, table = "diagnoses_icd", fields = fields, values = values)
  frameworkContext$inserts[[length(frameworkContext$inserts) + 1]] <- inserts
  invisible(NULL)
}
...
generateInsertSql <- function(databaseSchema = NULL) {
  ...
  insertSql <- c(insertSql, "TRUNCATE TABLE @cdm_database_schema.icustays;")
  insertSql <- c(insertSql, "TRUNCATE TABLE @cdm_database_schema.diagnoses_icd;")
