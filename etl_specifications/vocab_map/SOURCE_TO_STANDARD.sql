WITH CTE_VOCAB_MAP AS (
       ...
       FROM ohdsi_demo.concept C
             JOIN ohdsi_demo.concept_relationship CR
                        ON C.CONCEPT_ID = CR.CONCEPT_ID_1
                        AND CR.invalid_reason IS NULL
                        AND lower(cr.relationship_id) = 'maps to'
              JOIN ohdsi_demo.concept C1
       ...
       FROM ohdsi_demo.source_to_concept_map stcm
              LEFT OUTER JOIN ohdsi_demo.concept c1
                     ON c1.concept_id = stcm.source_concept_id
              LEFT OUTER JOIN ohdsi_demo.concept c2
                     ON c2.CONCEPT_ID = stcm.target_concept_id
       WHERE stcm.INVALID_REASON IS NULL
)
SELECT *
FROM CTE_VOCAB_MAP
WHERE LOWER(SOURCE_CODE_DESCRIPTION) IN ('emergency room')
AND TARGET_VOCABULARY_ID IN ('LOINC', 'SNOMED')